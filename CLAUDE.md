...
- **Also shipped (same day):** [docs/WHAT_WE_NEED.md](docs/WHAT_WE_NEED.md) — the full requirements list (what we need from Matt / from each manufacturer / from Industrial Supply as a company / from ourselves / legal-commercial), a viability re-read, and sequenced next steps by owner. Shareable version for Braydn + Matt: https://claude.ai/code/artifact/5dbf2170-0b3e-41b7-ae59-59b6f5ade97b
- **Viability call:** UP. The existential question ("can a distributor's real tier price be pulled programmatically?") is answered yes at brand #1. **The bottleneck moved from engineering to access administration** — every brand needs an account on the distributor's domain, at parent scope, blessed by a named rep. That is 50 rep conversations, it is the long pole, and it is also the moat: anyone can write the connector, almost nobody can open 50 manufacturer accounts.
- **New risk surfaced:** the PRD promises price + stock + lead time + ship-from. Brand #1's *best-case* rail gives price exactly, stock coarsely, and lead time / ship-from **not at all**. Recommendation: **lead with price** (provable today) and source inventory from a second rail (EDI 846 or their ERP). Do not demo on-hand quantities we cannot source.
- **Legal check (done, not assumed):** Milwaukee's public terms carry **no** anti-bot / anti-scraping / automated-access clause. They *do* reserve commercial exploitation to what is "expressly permitted ... in writing" — so get written blessing from Olivia in the same email as the parent-account ask. Caveat: that's the public site's legal page; a separate signed distributor agreement may exist.
